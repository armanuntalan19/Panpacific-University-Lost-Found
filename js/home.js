(async function () {
  ...
  async function loadRecentItems() {
    const container = el("recentItems");
    container.innerHTML = `<p class="loading-text">Loading recent items…</p>`;
    const items = await fetchItems({ limit: 6 });

    container.innerHTML = items.length
      ? items.map(renderItemCard).join("")
      : `<p class="loading-text">No items reported yet. Be the first!</p>`;
  }

  window.reloadItemsGrid = loadRecentItems;
  loadRecentItems();
})();
